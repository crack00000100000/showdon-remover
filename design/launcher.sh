...
if [ ! -x "$PYTHON" ]; then
    ...
    osascript <<EOF
display dialog "자막 제거 도구가 설치되어 있지 않거나 접근 권한이 없습니다.

가능한 원인:
1) install.command 를 아직 실행하지 않음
2) macOS 가 ~/Documents 접근을 차단함 (시스템 설정 → 개인정보 보호 → 파일 및 폴더에서 허용)

설치 위치: $INSTALL_DIR
로그: $LOG_FILE" buttons {"확인"} default button 1 with icon caution with title "자막 제거 도구"
EOF
    exit 1
fi
...
if [ ! -f "$INSTALL_DIR/gui.py" ]; then
    ...
    osascript -e 'display dialog "gui.py 를 찾을 수 없습니다." buttons {"확인"} default button 1 with icon stop with title "자막 제거 도구"'
    exit 1
fi
